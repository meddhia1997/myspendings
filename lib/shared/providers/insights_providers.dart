...
final categoryTotalsProvider =
    Provider.autoDispose<AsyncValue<List<CategoryTotal>>>((ref) {
...
final dailyTotalsProvider = Provider.autoDispose<AsyncValue<List<DailyTotal>>>((
  ref,
) {
  final transactionsAsync = ref.watch(monthlyTransactionsProvider);
  ...
  return transactionsAsync.whenData((transactions) {
    ...
    return [
      for (var d = 1; d <= daysInMonth; d++)
        DailyTotal(
          day: d,
          expenseMinor: expenseByDay[d],
          incomeMinor: incomeByDay[d],
        ),
    ];
  });
});
